...
// ============================================================================
// Mesh Normalizer - PCA-based Mesh Alignment (CPU Version)
...
public:
    ...
    static NormalizationResult normalize(MeshSoA& mesh) {
        ...
        if (spread_bottom > spread_top * 1.2f) {  // 20% threshold
            ...
            // Rotate 180 degrees around X axis: negate Y and Z coordinates
            for (uint32_t i = 0; i < n; ++i) {
                mesh.vertices_y[i] = -mesh.vertices_y[i];
                mesh.vertices_z[i] = -mesh.vertices_z[i];
                if (mesh.normals_y) mesh.normals_y[i] = -mesh.normals_y[i];
